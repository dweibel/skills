...
## PDFs Generated by Pandoc and XeLaTeX

PDFs built by the `pdf-build` skill, or by any Pandoc + XeLaTeX pipeline, defeat two of the default recipes above.

**Words run together.** XeTeX places glyphs tightly, so `extract_text()` returns `UltimaterealityandKingdom`. Pass a smaller horizontal tolerance and the spaces come back:

```python
text = page.extract_text(x_tolerance=1)
```

Use the same argument with `layout=True`. Any PDF whose producer metadata says `xdvipdfmx` or `XeTeX` needs this.

**Tables are not detected.** Pandoc emits `booktabs` tables, which have horizontal rules only. The default line-based detection in `extract_tables()` finds zero tables, and the text-based strategy (`{"vertical_strategy": "text", "horizontal_strategy": "text"}`) returns one row per text line with the columns merged, which is worse than the raw text. When the Markdown source exists, read the table from the source. When only the PDF exists, extract the page text with `x_tolerance=1` and split rows on the known column headers.

**PyMuPDF as the alternative.** If the environment already has PyMuPDF (the `pdf-build` QA script depends on it), its text extraction handles XeTeX spacing without tuning:
...
Its `find_tables()` has the same blind spot for rule-less tables.

**Where pdfplumber lives.** `pip install pdfplumber` fails on Homebrew Python outside a virtual environment. If a `~/.venvs/pdf-qa` environment exists from `pdf-build`, install there and run with that interpreter:

```bash
~/.venvs/pdf-qa/bin/pip install pdfplumber
~/.venvs/pdf-qa/bin/python script.py
```
